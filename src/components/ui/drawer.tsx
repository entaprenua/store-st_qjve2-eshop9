...
const DrawerCloseButton = DrawerPrimitive.Close
...
const DrawerOverlay = <T extends ValidComponent = "div">(
  ...
  return (
    <DrawerPrimitive.Overlay
      ...
      style={{
        "background-color": `rgb(0 0 0 / ${0.3 * drawerContext.openPercentage()})`
      }}
      {...rest}
    />
  )
...
const DrawerContent = <T extends ValidComponent = "div">(
  rawProps: DynamicProps<T, DrawerContentProps<T>>
) => {
  const props = mergeProps({
    showBackdrop: true,
  }, rawProps) 
  const [, rest] = splitProps(props as DrawerContentProps, ["class", "showBackdrop", "children"])
  return (
    <DrawerPortal>
      <Show when={props.showBackdrop}> 
        <DrawerOverlay />
      </Show> 
